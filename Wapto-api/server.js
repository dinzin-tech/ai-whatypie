...
import seedBrandingAssets from './seeder/seedBrandingAssets.js';
import seedRebrandLandingPage from './seeder/seedRebrandLandingPage.js';
...
(async () => {
  try {
    ...
    httpServer.listen(PORT, () => {
      ...
      import('./queues/campaign-queue.js').then(({ getCampaignQueue, getCampaignWorker }) => {
        getCampaignQueue();
        getCampaignWorker();
        console.log('Campaign queue worker initialized');
      }).catch(err => console.error('Error starting campaign queue worker', err));

      import('./queues/drip-campaign-queue.js').then(({ getDripCampaignQueue, getDripCampaignWorker }) => {
        getDripCampaignQueue();
        getDripCampaignWorker();
        console.log('Drip campaign queue worker initialized');
      }).catch(err => console.error('Error starting drip campaign queue worker', err));
    });
  } catch (err) {
