...
import { formatWhatsAppGraphError, logWhatsAppGraphError } from '../../../utils/whatsapp-graph-error.js';
import {
...
export default class BusinessAPIProvider extends BaseProvider {
  ...
  async sendWhatsAppAPIMessage(params) {
    ...
    if (!response.ok) {
      logWhatsAppGraphError('sendWhatsAppAPIMessage', responseData, response.status, {
        phone_number_id,
        to: payload?.to,
        templateName: payload?.template?.name,
        hasAccessToken: Boolean(access_token)
      });
      throw new Error(`WhatsApp API error: ${formatWhatsAppGraphError(responseData, response.status)}`);
    }
  ...
  async processMediaUpload(params, userId = null) {
    ...
    const isVoiceNote = isLikelyChatVoiceNote(file);

    if (messageType === MESSAGE_TYPES.AUDIO) {
      assertWhatsAppAllowedAudioMime(file.mimetype);
    }

    const uploadMimeType = messageType === MESSAGE_TYPES.AUDIO
      ? getWhatsAppAudioUploadMime(file.mimetype, { isVoiceNote })
      : file.mimetype;
  ...
  getPublicMediaUrl(filePath) {
    ...
    if (String(filePath).startsWith('blob:')) {
      throw new Error('Invalid media URL: blob URLs are local browser references and must be uploaded first.');
    }
