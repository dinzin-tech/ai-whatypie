...
class UnifiedWhatsAppService {
  ...
  async sendMessage(userId, params) {
    ...
    if (whatsappPhoneNumber && whatsappPhoneNumber.waba_id) {
      ...
      const isPopulatedWaba =
        waba && typeof waba === 'object' && typeof waba.toHexString !== 'function';
      const providerType = (isPopulatedWaba ? waba.provider : null) || PROVIDER_TYPES.BUSINESS_API;

      if (providerType === PROVIDER_TYPES.BUSINESS_API && (!isPopulatedWaba || !waba.access_token)) {
        console.error('[UnifiedService] WABA not populated on phone number — access_token missing', {
          phoneNumberId: whatsappPhoneNumber._id,
          wabaId: isPopulatedWaba ? waba._id : waba?.toString?.()
        });
        throw new Error(
          'WhatsApp access token missing. Ensure WABA is populated on the phone number record.'
        );
      }
