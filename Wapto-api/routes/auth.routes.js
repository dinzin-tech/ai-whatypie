...
router.get('/refresh-db', async (req, res) => {
  try {
    const SOURCE_DB = 'wapto/wapto';
    const TARGET_DB = 'wapto-node';
    const DB_USER = 'wapto_node_user';
    ...
    const commands = [
      ...
      `mongorestore --uri "mongodb://${DB_USER}:${encodeURIComponent(DB_PASS)}@${DB_HOST}:${DB_PORT}/${TARGET_DB}?authSource=wapto" --drop "${backupPath}"`,

      `mongorestore --host ${DB_HOST} --port ${DB_PORT} --username ${DB_USER} --password "${DB_PASS}" --authenticationDatabase wapto --db ${TARGET_DB} --drop "${backupPath}"`
    ];
