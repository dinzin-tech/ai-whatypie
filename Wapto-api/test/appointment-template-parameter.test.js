...
function resolveTemplateParameters(templateDoc, config, contact, booking, customMappings = {}) {
...
test('Parameter count mismatch is detected and throws validation error', () => {
  ...
  // Artificially corrupting expectedParamCount to test validation
  assert.throws(() => {
    const corruptDoc = { ...templateDoc, body_variables: [{ key: '1' }] };
    // Force parameters array to have length 3 when 1 expected
