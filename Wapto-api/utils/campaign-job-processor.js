...
const refreshCampaignProgress = async (campaignId) => {
  ...
  if (!campaignStats?.recipients) return;
  ...
  if (pendingCount === 0) {
    ...
    if (campaignStats.sent_at) {
      ...
      updateData.completion_duration_seconds = Math.round(
        (updateData.completed_at - startTime) / 1000
      );
    }
  }
...
export const processCampaignMessageJob = async (jobData) => {
  ...
  try {
    ...
    if (updateResult.modifiedCount > 0) {
      await refreshCampaignProgress(campaign._id);
    }
