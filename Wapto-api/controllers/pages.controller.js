...
export const getPublicPageBySlug = async (req, res) => {
    try {
        ...
        const page = await Page.findOne({ slug, status: true, deleted_at: null }).lean();
...
const RESERVED_SLUGS = new Set(['terms-and-conditions', 'privacy-policy', 'refund-policy']);
...
export const updatePage = async (req, res) => {
    try {
        ...
        const isReservedSlug = RESERVED_SLUGS.has(page.slug);
        if (!isReservedSlug && newSlug !== page.slug) {
