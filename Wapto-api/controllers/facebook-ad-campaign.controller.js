...
export const getFbAdInsights = async (req, res) => {
  try {
    ...
    if (level === 'campaign') {
      adAccountId = localDoc.ad_account_id;
    } else if (level === 'adset') {
      const campaign = await FacebookAdCampaign.findById(localDoc.campaign_id).select('ad_account_id').lean();
      adAccountId = campaign?.ad_account_id;
    } else if (level === 'ad') {
      const adset = await FacebookAdSet.findById(localDoc.ad_set_id).select('campaign_id').lean();
      const campaign = adset
        ? await FacebookAdCampaign.findById(adset.campaign_id).select('ad_account_id').lean()
        : null;
      adAccountId = campaign?.ad_account_id;
    }
    ...
    let currency = 'USD';
    if (adAccountId) {
      const account = await FacebookAdAccount.findOne({
        user_id: userId,
