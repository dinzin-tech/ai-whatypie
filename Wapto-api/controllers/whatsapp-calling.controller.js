import { WhatsappCallSetting, WhatsappCallAgent, WhatsappCallLog, Contact } from '../models/index.js';
import whatsappCallingService from '../services/whatsapp/whatsapp-calling.service.js';

const resolveOwnerId = (req) => req.user.owner_id || req.user._id;
const storagePhoneId = (phone) => String(phone._id);
...
class WhatsappCallingController {
    async getCallSettings(req, res) {
        try {
            ...
            if (!phone_number_id) {
                return res.status(400).json({ message: 'phone_number_id is required' });
            }

            const phone = await whatsappCallingService.findPhoneRecord(phone_number_id, ownerId);
            if (!phone) {
                return res.status(404).json({ message: 'Phone number not found' });
            }

            const phoneKey = storagePhoneId(phone);

            let settings = await WhatsappCallSetting.findOne({
                user_id: ownerId,
                deleted_at: null,
                $or: [
                    { phone_number_id: phoneKey },
            ...
            if (!settings) {
                try {
                    const metaResponse = await whatsappCallingService.getCallConfigFromMeta(phone);
                    const mapped = whatsappCallingService.mapMetaCallingToSettings(
                        metaResponse?.calling,
                        phoneKey,
                        ownerId
                    );
                    settings = await WhatsappCallSetting.create(mapped);
                } catch (metaError) {
                    settings = await WhatsappCallSetting.create({
                        ...
                        sip_config: { status: 'DISABLED', servers: [] }
                    });
                    console.warn('Could not fetch call settings from Meta, using defaults:', metaError.message);
                }
                await settings.populate('fallback_agent_id');
            }
    ...
    async updateCallSettings(req, res) {
        try {
            ...
            try {
                await whatsappCallingService.updateCallConfig(phone, settings);
            } catch (syncError) {
