...
# ==============================================================================
# Wapto Automated Installation Script (Multi-Project Version)
...
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}      Wapto Multi-Project Installation Script        ${NC}"
echo -e "${GREEN}====================================================${NC}"
...
echo -e "\n${YELLOW}--- Selection ---${NC}"
read -p "Install Wapto Backend? (y/n): " INSTALL_BACKEND
read -p "Install Wapto Frontend? (y/n): " INSTALL_FRONTEND
read -p "Install Wapto Admin Dashboard? (y/n): " INSTALL_ADMIN


if [[ "$INSTALL_BACKEND" == "y" ]]; then
    read -p "Enter Wapto BACKEND Git URL: " BACKEND_REPO
fi
if [[ "$INSTALL_FRONTEND" == "y" ]]; then
    read -p "Enter Wapto FRONTEND Git URL: " FRONTEND_REPO
fi
if [[ "$INSTALL_ADMIN" == "y" ]]; then
    read -p "Enter Wapto ADMIN Git URL: " ADMIN_REPO
fi
...
if [[ "$IS_PRIVATE" == "y" ]]; then
    if [ ! -f ~/.ssh/id_rsa.pub ] && [ ! -f ~/.ssh/id_ed25519.pub ]; then
        ...
        ssh-keygen -t ed25519 -C "wapto-server" -f ~/.ssh/id_ed25519 -N ""
...
setup_project() {
    ...
    if [[ "$TYPE" == "backend" ]]; then
        sed -i "s|MONGODB_URI=.*|MONGODB_URI=mongodb://$MONGO_USER:$MONGO_PASS@127.0.0.1:27017/wapto?authSource=admin|g" .env
...
[[ "$INSTALL_BACKEND" == "y" ]] && setup_project "wapto-backend" "$BACKEND_REPO" "backend"
[[ "$INSTALL_FRONTEND" == "y" ]] && setup_project "wapto-frontend" "$FRONTEND_REPO" "frontend"
[[ "$INSTALL_ADMIN" == "y" ]] && setup_project "wapto-admin" "$ADMIN_REPO" "admin"
