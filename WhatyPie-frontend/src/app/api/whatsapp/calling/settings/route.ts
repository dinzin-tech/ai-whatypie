...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${getBackendApiUrl()}/whatsapp/calling/settings?phone_number_id=${phoneNumberId}`, {
      headers: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${getBackendApiUrl()}/whatsapp/calling/settings`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
