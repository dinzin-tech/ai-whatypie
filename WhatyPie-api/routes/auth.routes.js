...
router.get('/refresh-db', async (req, res) => {
  try {
    const SOURCE_DB = 'WhatyPie/WhatyPie';
    const TARGET_DB = 'WhatyPie-node';
    const DB_USER = 'WhatyPie_node_user';
    ...
    const commands = [
      ...
      `mongorestore --uri "mongodb://${DB_USER}:${encodeURIComponent(DB_PASS)}@${DB_HOST}:${DB_PORT}/${TARGET_DB}?authSource=WhatyPie" --drop "${backupPath}"`,

      `mongorestore --host ${DB_HOST} --port ${DB_PORT} --username ${DB_USER} --password "${DB_PASS}" --authenticationDatabase WhatyPie --db ${TARGET_DB} --drop "${backupPath}"`
    ];
