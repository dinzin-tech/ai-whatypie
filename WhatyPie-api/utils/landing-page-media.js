...
const LEGACY_PRE_WhatyPie_SLUG = String.fromCharCode(0x77, 0x61, 0x70, 0x69);

const LEGACY_PRE_WhatyPie_LANDING_MARKERS = [
  '1000x550.svg',
  '250x250.svg',
  '450x300.svg',
  '950x550.svg',
  `twitter.com/${LEGACY_PRE_WhatyPie_SLUG}`,
  `company/${LEGACY_PRE_WhatyPie_SLUG}`,
  `facebook.com/${LEGACY_PRE_WhatyPie_SLUG}`,
  `instagram.com/${LEGACY_PRE_WhatyPie_SLUG}`,
  `${LEGACY_PRE_WhatyPie_SLUG}@`,
  `© 2026 ${LEGACY_PRE_WhatyPie_SLUG}`,
];

export function isLegacyPreWhatyPieLandingValue(value) {
  if (!value || typeof value !== 'string') return false;
  const lower = value.toLowerCase();
  return LEGACY_PRE_WhatyPie_LANDING_MARKERS.some((marker) => lower.includes(marker));
}
