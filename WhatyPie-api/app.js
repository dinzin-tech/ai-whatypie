...
import dripCampaignRoutes from "./routes/drip-campaign.routes.js";
...
app.get("/api/is-demo-mode", async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      is_demo_mode,
      logo_light_url: normalizeUploadPath(setting?.logo_light_url),
      logo_dark_url: normalizeUploadPath(setting?.logo_dark_url),
      favicon_url: normalizeUploadPath(setting?.favicon_url),
      ...(is_demo_mode
        ? {
...
app.post("/webhook/whatsapp", (req, res) => {
  ...
  if (value?.statuses) {
    ...
    console.log('Unknown WhatsApp webhook type:', JSON.stringify(req.body, null, 2));
    return res.sendStatus(200);
  }
});
