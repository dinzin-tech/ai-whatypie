...
import fs from 'fs';
import { normalizeStoredPhone, toE164Digits } from '../utils/phone-normalization.js';
...
const validatePhoneNumber = (phoneNumber, countryCodeHint = null) => {
  const cleaned = countryCodeHint
    ? toE164Digits(countryCodeHint, phoneNumber)
    : normalizeStoredPhone(phoneNumber);
...
export const createContact = async (req, res) => {
  try {
    ...
    const rawPhone = phone_number || (bodyCountryCode && bodyPhone ? `${bodyCountryCode}${bodyPhone}` : null);
    ...
    const phoneValidation = validatePhoneNumber(rawPhone, bodyCountryCode || null);
    if (!phoneValidation.isValid) {
      return res.status(400).json({
        success: false,
    ...
    if (softDeletedContact) {
      ...
      softDeletedContact.name = name.trim();
      softDeletedContact.source = source;
      softDeletedContact.assigned_to = assigned_to || null;
      softDeletedContact.tags = normalizedTags.length ? normalizedTags : softDeletedContact.tags;
      softDeletedContact.email = email ? email.trim() : softDeletedContact.email;
      softDeletedContact.status = status;
      softDeletedContact.custom_fields = custom_fields;
      softDeletedContact.updated_by = req.user.id;
      await softDeletedContact.save();

      if (req.body.segments && Array.isArray(req.body.segments)) {
        await segmentService.updateContactSegments(softDeletedContact._id, req.body.segments, userId);
      }

      return res.status(201).json({
        success: true,
        message: 'Contact restored successfully',
        data: softDeletedContact
      });
    }
    ...
    if (error?.code === 11000) {
      ...
      const cleanedPhoneNumber = phoneValidation.isValid ? phoneValidation.cleanedNumber : req.body.phone_number;
      ...
      if (existing) {
        ...
        return res.status(201).json({
          success: true,
          message: wasDeleted ? 'Contact restored successfully' : 'Contact already exists',
          data: existing
        });
      }
    }
...
export const updateContact = async (req, res) => {
  try {
    ...
    if (updateData.phone_number) {
      const countryHint = updateData.country_code || null;
      const phoneValidation = validatePhoneNumber(updateData.phone_number, countryHint);
      if (!phoneValidation.isValid) {
        return res.status(400).json({
          success: false,
          message: phoneValidation.message
