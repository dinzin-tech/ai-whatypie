...
import { clearDemoModeCache } from '../middlewares/demo-mode.js';
...
const getAllSettings = async (req, res) => {
  ...
  for (const field of logoFields) {
    ...
    const webPath = normalizeUploadPath(out[field]);
    out[field] = webPath;
    if (webPath.startsWith('/uploads/') && !isValidUrl(webPath)) {
      out[field] = `${baseUrl}${webPath}`;
    }
  }
  ...
  try {
    ...
    for (const field of logoFields) {
      ...
      if (uploadedFile) {
        ...
        if (file) {
          processedBody[field] = toWebUploadPath(file.path);
        }
      }
    ...
    if (req.body.enabled_connection_methods !== undefined) {
      if (typeof req.body.enabled_connection_methods === 'string') {
        try {
          processedBody.enabled_connection_methods = JSON.parse(req.body.enabled_connection_methods);
        } catch {
          processedBody.enabled_connection_methods = req.body.enabled_connection_methods
            .split(',')
            .map(item => item.trim())
            .filter(item => item);
        }
    ...
    if (req.body.facebook_lead_webhook_verify_token !== undefined) {
      processedBody.facebook_lead_webhook_verify_token = req.body.facebook_lead_webhook_verify_token;
    ...
    if (req.body.is_demo_mode !== undefined || processedBody.is_demo_mode !== undefined) {
      clearDemoModeCache();
    }
