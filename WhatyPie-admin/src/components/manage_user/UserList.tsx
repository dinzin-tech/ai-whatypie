...
const WhatyPie_FRONT_URL = process.env.NEXT_PUBLIC_WhatyPie_FRONT_URL || "http://localhost:3000";
...
const UserList = ({
  ...
  const handleImpersonate = async (user: User) => {
    try {
      ...
      window.location.href = `${WhatyPie_FRONT_URL}/auth/impersonate?token=${result.token}`;
    } catch (err: any) {
