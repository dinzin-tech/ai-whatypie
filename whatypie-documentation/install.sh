...
# ==============================================================================
# WhatyPie Automated Installation Script (Multi-Project Version)
...
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}      WhatyPie Multi-Project Installation Script        ${NC}"
echo -e "${GREEN}====================================================${NC}"
...
echo -e "\n${YELLOW}--- Selection ---${NC}"
read -p "Install WhatyPie Backend? (y/n): " INSTALL_BACKEND
read -p "Install WhatyPie Frontend? (y/n): " INSTALL_FRONTEND
read -p "Install WhatyPie Admin Dashboard? (y/n): " INSTALL_ADMIN


if [[ "$INSTALL_BACKEND" == "y" ]]; then
    read -p "Enter WhatyPie BACKEND Git URL: " BACKEND_REPO
fi
if [[ "$INSTALL_FRONTEND" == "y" ]]; then
    read -p "Enter WhatyPie FRONTEND Git URL: " FRONTEND_REPO
fi
if [[ "$INSTALL_ADMIN" == "y" ]]; then
    read -p "Enter WhatyPie ADMIN Git URL: " ADMIN_REPO
fi
...
if [[ "$IS_PRIVATE" == "y" ]]; then
    if [ ! -f ~/.ssh/id_rsa.pub ] && [ ! -f ~/.ssh/id_ed25519.pub ]; then
        ...
        ssh-keygen -t ed25519 -C "WhatyPie-server" -f ~/.ssh/id_ed25519 -N ""
...
setup_project() {
    ...
    if [[ "$TYPE" == "backend" ]]; then
        sed -i "s|MONGODB_URI=.*|MONGODB_URI=mongodb://$MONGO_USER:$MONGO_PASS@127.0.0.1:27017/WhatyPie?authSource=admin|g" .env
...
[[ "$INSTALL_BACKEND" == "y" ]] && setup_project "WhatyPie-backend" "$BACKEND_REPO" "backend"
[[ "$INSTALL_FRONTEND" == "y" ]] && setup_project "WhatyPie-frontend" "$FRONTEND_REPO" "frontend"
[[ "$INSTALL_ADMIN" == "y" ]] && setup_project "WhatyPie-admin" "$ADMIN_REPO" "admin"
