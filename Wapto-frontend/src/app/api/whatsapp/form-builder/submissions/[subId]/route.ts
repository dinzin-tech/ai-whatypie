...
export async function GET(request: NextRequest, { params }: { params: Promise<{ subId: string }> }) {
  try {
    ...
    const response = await fetch(`${getBackendApiUrl()}/submissions/${subId}/details`, {
      headers: {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ subId: string }> }) {
  try {
    ...
    const response = await fetch(`${getBackendApiUrl()}/submissions/${subId}/status`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ subId: string }> }) {
  try {
    ...
    const response = await fetch(`${getBackendApiUrl()}/submissions/${subId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
